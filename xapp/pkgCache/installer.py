...
from xapp.pkgCache import cache, _flatpak, _apt
...
class Installer:
    ...
    def execute_task(self, task, client_finished_cb, client_progress_cb=None, client_error_cb=None):
        ...
        task.finished_cleanup_cb = self._post_task_update
    ...
    def _post_task_update(self, task):
        ...
        if task.pkginfo.pkg_hash.startswith("a"):
            thread = threading.Thread(target=self._apt_post_trans_update_cache_thread, args=(task,))
        else:
            thread = threading.Thread(target=self._flatpak_post_trans_update_cache_thread, args=(task,))

        thread.start()

    def _apt_post_trans_update_cache_thread(self, task):
        _apt._sync_cache_installed_states(self.cache, task)

        if task.client_finished_cb:
            GObject.idle_add(task.client_finished_cb, task.pkginfo)

    def _flatpak_post_trans_update_cache_thread(self, task):
        if task.client_finished_cb:
            GObject.idle_add(task.client_finished_cb, task.pkginfo)
